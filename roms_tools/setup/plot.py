...
def _plot_nesting(parent_grid_ds, child_grid_ds, parent_straddle):
    ...
    trans = ccrs.NearsidePerspective(
        central_longitude=parent_lon_deg.mean().values,
        central_latitude=parent_lat_deg.mean().values,
    )
    ...
    # transform coordinates to projected space
    transformed_parent_corners = [
        trans.transform_point(lo, la, proj) for lo, la in parent_corners
    ]
    transformed_parent_lons, transformed_parent_lats = zip(*transformed_parent_corners)
    transformed_child_corners = [
        trans.transform_point(lo, la, proj) for lo, la in child_corners
    ]
